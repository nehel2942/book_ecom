...
export const getSingleProductController = async (req,res) => {
    try {
        ...
        res.status(200).send({
            success: true,
            message:"This product is fetched successfully!",
            product,
        })
    } catch (error) {
        console.log(error)
...
export const updateProductController = async (req, res) => {
    try {
        const {name, slug, description, pages, price, category, quantity, shipping} = req.fields
        const {photo} = req.files
        ...
        switch(true){
            ...
            case !pages:
                return res.status(500).send({error: "Product Pages is required."})
            case !price:
        ...
        const {pid} = req.params
        const product = await productModel.findOne({pid}).select("-photo")
        product.name = name
        ...
        res.status(200).send({
            success: true,
            message:"This product is updated successfully!",
            product,
        })
    } catch (error) {
        console.log(error)
        res.status(500).send({
            success: false,
            error,
            message: "Error while updating this product"
        })
    }
}
